<template>
  <div class="upload-excel-container">
    ...
    <div
      ...
      @drangenter.prevent.stop="onDropEnter"
      @dragover.prevent.stop="onDropOver"
    >
      <svg-icon iconName="云端上传" class="i"></svg-icon>
...
const handleChange = (e) => {
  ...
  // 插件  解析 excel文件的表格数据 -->data
  // props.onSuccess('解析完的数据')
}

// 执行上传  如果存在beforupload 而且beforeupload 返回true 才执行操作 如果没有beforeupload  直接读取操作
...
const checkFileType = (rawFile) => {
  if (type.value === 'excel') {
    if (!isExcel(rawFile)) {
      ElMessage.error(rawFile.name + ':' + i18n.t('msg.excel.noExcelFile'))
      return false
    } else {
      return true
    }
    ...
    if (!isJs(rawFile)) {
      ElMessage.error(rawFile.name + ':' + i18n.t('msg.excel.noJsFile'))
      return false
    } else {
      return true
    }
